...
function App() {
  ...
  const cargarPerfil = async (u) => {
    ...
    if (!snap.exists()) {
      const nuevoPerfil = {
        uid: u.uid,
        email: u.email,
        nombre: authForm.nombre || u.email,
  ...
  const cargarUsuariosAdmin = async () => {
    try {
      ...
      data.sort((a, b) => String(b.creado || "").localeCompare(String(a.creado || "")));
  ...
  const activarUsuario30Dias = async (user) => {
    if (!confirm(`¿Activar 30 días a ${user.email}?`)) return;
  ...
  const activarUsuario90Dias = async (user) => {
    if (!confirm(`¿Activar 90 días a ${user.email}?`)) return;

    await updateDoc(doc(db, "usuarios", user.id), {
      activo: true,
      fechaVencimiento: sumarDias(hoyISO(), 90)
    });
  ...
  const hacerAdmin = async (user) => {
    if (!confirm(`¿Convertir en admin a ${user.email}?`)) return;
  ...
  const guardarConfiguracion = () => {
    ...
    const telefonoFinal = nuevoTelefonoAdmin.trim() || config.telefonoAdmin;

    setConfig({ ...config, nombreApp: nombreFinal, telefonoAdmin: telefonoFinal });
    setNuevoNombreApp("");
  ...
  const marcarPago = async (cliente) => {
    ...
    await updateDoc(doc(db, "usuarios", usuario.uid, "clientes", cliente.id), {
  ...
  const vaciarPapelera = async () => {
    ...
    for (const p of papelera) {
      await deleteDoc(doc(db, "usuarios", usuario.uid, "papelera", p.id));
    }
    await cargarTodo(usuario.uid);
  };
...
🙏 Gracias por su pago.`;
  ...
  const usuariosActivos = usuariosAdmin.filter((u) => u.activo !== false && hoyISO() <= (u.fechaVencimiento || "2000-01-01")).length;
  const usuariosVencidos = usuariosAdmin.filter((u) => u.rol !== "admin" && (u.activo === false || hoyISO() > (u.fechaVencimiento || "2000-01-01"))).length;
      ...
      <div className="loginPage">
        <div className="loginBox">
          ...
          <input
            ...
            onKeyDown={(e) => e.key === "Enter" && (modoAuth === "login" ? iniciarSesion() : registrarUsuario())}
          />
          ...
          <button className="gris" onClick={() => setModoAuth(modoAuth === "login" ? "registro" : "login")}>
            {modoAuth === "login" ? "Crear cuenta nueva" : "Ya tengo cuenta"}
          </button>
    ...
    <div className="app">
      <header className="header">
        ...
        <div className="headerBtns">
          {esAdmin && <button className="adminBtn" onClick={() => setMostrarAdmin(!mostrarAdmin)}>Admin</button>}
      ...
      {esAdmin && mostrarAdmin && (
        <section className="card adminPanel">
          ...
          <section className="grid resumen">
            <Box titulo="Usuarios" valor={usuariosAdmin.length} />
            <Box titulo="Activos" valor={usuariosActivos} />
            <Box titulo="Vencidos" valor={usuariosVencidos} />
          </section>

          <button className="gris" onClick={cargarUsuariosAdmin}>Actualizar usuarios</button>

          {usuariosAdmin.map((u) => {
            const vencidoUsuario = u.rol !== "admin" && (u.activo === false || hoyISO() > (u.fechaVencimiento || "2000-01-01"));

            return (
              <div key={u.id} className="usuarioAdmin">
                <h3>{u.nombre || "Sin nombre"}</h3>
                <p><b>Email:</b> {u.email}</p>
                <p><b>Rol:</b> {u.rol || "cliente"}</p>
                <p><b>Vence:</b> {u.fechaVencimiento || "Sin fecha"}</p>
                <p><b>Estado:</b> <span className={vencidoUsuario ? "textoRojo" : "textoVerde"}>{vencidoUsuario ? "Vencido/Bloqueado" : "Activo"}</span></p>

                {u.rol !== "admin" && (
                  <>
                    <button className="verdeBtn" onClick={() => activarUsuario30Dias(u)}>Activar 30 días</button>
                    <button className="azulBtn" onClick={() => activarUsuario90Dias(u)}>Activar 90 días</button>
                    <button className="rojoBtn" onClick={() => bloquearUsuario(u)}>Bloquear</button>
                    <button className="gris" onClick={() => hacerAdmin(u)}>Hacer admin</button>
                  </>
                )}
              </div>
            );
      ...
      {mostrarConfig && (
        <section className="card">
          ...
          <input placeholder={config.telefonoAdmin} value={nuevoTelefonoAdmin} onChange={(e) => setNuevoTelefonoAdmin(e.target.value)} />

          <button onClick={guardarConfiguracion}>Guardar configuración</button>
          <p className="nota">Estos datos se guardan en este dispositivo.</p>
        </section>
      )}
      ...
      {vencida && (
        <section className="card">
          ...
          <p>Tu suscripción venció. Para renovar, presiona el botón de WhatsApp.</p>
        </section>
      )}

      {!vencida && (resumen.pagosHoy > 0 || resumen.atrasados > 0) && (
      ...
      {!vencida && (
        <>
          ...
          <div className="accionesTop">
            ...
            <button className="gris" onClick={() => cargarTodo(usuario.uid)}>Actualizar nube</button>
...
.salir,.configBtn,.adminBtn{width:auto;padding:9px 13px;background:#334155;margin-top:0}
.configBtn{background:#2563eb}
.adminBtn{background:#ca8a04}
...
.adminPanel{border:2px solid #facc15}
.usuarioAdmin{background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:12px;margin-top:12px}
.usuarioAdmin h3{margin:0 0 6px}
.usuarioAdmin p{margin:5px 0}
.textoRojo{color:#dc2626;font-weight:bold}
.textoVerde{color:#16a34a;font-weight:bold}
